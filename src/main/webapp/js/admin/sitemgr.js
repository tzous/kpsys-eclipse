                ...
                columns : [ [ {
                    ...
                    formatter : function(value, row, index) {
                        if(row.ICONCLS && row.ICONCLS.length>0){
                            ...
                            var s = '<span  onclick="dataEdit(\''+row.siteid+'\');" style="color:blue;">编辑</span> '
                                +'| <span  onclick="dataDelete(\''+row.siteid+'\');" style="color:blue;">删除</span>';
                            return s ;
                        }
                    }} ] ],
                toolbar : [ {
                    ...
                    iconCls : 'icon-add',
                    handler : function() {
                        $("#iframemain").attr("src", "admin/siteedit.do");
                        $("#wedit").window('open');
                    }
                }, {
                    id : 'btncut',
...
function dataEdit(icode){
    $("#iframemain").attr("src", "admin/siteedit.do?siteid="+icode);
    $("#wedit").window('open');
}
...
function dataDelete(icode){
    $.messager.confirm('询问','确定要删除'+ icode + '吗?',function(r){
        if (r){
            $.ajax( {
                type : "post",
                url : "admin/SiteDelete.do?siteid="+icode,
